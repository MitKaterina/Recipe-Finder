...
function showRecipeDetails(recipeId) {
    window.location.href = `single-post.html?id=${recipeId}`; // Переход на страницу с деталями рецепта и добавление параметра id
}
...
async function loadRecipeDetails() {
    ...
    try {
        ...
        const recipe = await response.json(); // Преобразование ответа в формат JSON
